...
class PreflopPlayer(BasePokerPlayer):
    ...
    def select_action(self, win_rate, round_state, on_the_big_blind, on_the_small_blind, current_players, valid_actions,
                      stack, current_players_uuids):
        ...
        # aggressiveness = [(self.players_stats[x].actions['FOLD'])/
        #                   (self.players_stats[x].actions_count+1) for x in current_players_uuids]
        #
        # count_agressive = len(list(filter(lambda x: x < 0.5, aggressiveness)))
        action = FOLD

        if win_rate >= 0.75 and (round_state['street'] == 'river' or round_state['street'] == 'turn'):
            action = MAX_RAISE

        elif stack / self.start_stack <= 0.3 and win_rate >= 0.65:
            action = MAX_RAISE

        elif round_state['street'] == 'preflop' and win_rate >= 1.5 / current_players and valid_actions[1][
            'amount'] / stack < 0.5:
            action = CALL
        ...
        elif round_state['street'] == 'preflop' and (
            on_the_big_blind or on_the_small_blind) and win_rate >= 1 / current_players and valid_actions[1][
            'amount'] / stack < 0.35:
            action = CALL

        elif round_state[
            'street'] == 'flop' and self.previous_action == MIN_RAISE and self.previous_street == 'flop' and \
                                valid_actions[1]['amount'] / stack < 0.65:
            action = CALL

        elif round_state['street'] == 'flop' and win_rate >= 1.5 / current_players and valid_actions[1][
            'amount'] / stack < 0.65:
            action = MIN_RAISE
        ...
        elif round_state['street'] == 'flop' and win_rate >= 1 / current_players and valid_actions[1][
            'amount'] / stack < 0.5:
            action = CALL

        elif round_state[
            'street'] == 'turn' and self.previous_action == MIN_RAISE and self.previous_street == 'turn' and \
                                valid_actions[1]['amount'] / stack < 0.65:
            action = CALL

        elif round_state['street'] == 'turn' and win_rate >= 1.5 / current_players and valid_actions[1][
            'amount'] / stack < 0.65:
            action = MIN_RAISE

        elif round_state['street'] == 'turn' and win_rate >= 1 / current_players and valid_actions[1][
            'amount'] / stack < 0.5:
            action = CALL
            ...
            'street'] == 'river' and self.previous_action == MIN_RAISE and self.previous_street == 'river' and \
                                valid_actions[1]['amount'] / stack < 0.65:
            action = CALL

        elif round_state[
            'street'] == 'river' and win_rate >= 1.6 / current_players and on_the_big_blind and count_allins == 0:
            action = MAX_RAISE

        elif round_state['street'] == 'river' and win_rate >= 1.5 / current_players and valid_actions[1][
            'amount'] / stack < 0.65:
            action = MIN_RAISE

        elif round_state[
            'street'] == 'river' and win_rate >= 1 / current_players and self.round > 25 and on_the_big_blind \
                and valid_actions[1]['amount'] == 0 and count_folds / count_actions > 0.85:
            action = MIN_RAISE

        elif round_state['street'] == 'river' and win_rate >= 1 / current_players and valid_actions[1][
            'amount'] / stack < 0.5:
            action = CALL
